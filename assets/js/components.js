...
// Mengatur navbar, footer, favicon, stylesheet audit, dan auth UI.
// =========================================================
...
function loadFooterStyles() { if (!document.querySelector("link[data-footer-style]")) { const link=document.createElement("link"); link.rel="stylesheet"; link.href="assets/css/footer.css"; link.dataset.footerStyle="true"; document.head.appendChild(link); } }
function loadThemeAuditStyles() { if (!document.querySelector("link[data-theme-audit-style]")) { const link=document.createElement("link"); link.rel="stylesheet"; link.href="assets/css/final-theme-audit.css"; link.dataset.themeAuditStyle="true"; document.head.appendChild(link); } }
function loadBrandIcon() { if (!document.querySelector("link[data-brand-icon]")) { const link=document.createElement("link"); link.rel="icon"; link.type="image/png"; link.href="assets/img/logo-sics-j4-transparent.png"; link.dataset.brandIcon="true"; document.head.appendChild(link); } }

function mountComponents() {
  loadFooterStyles(); loadThemeAuditStyles(); loadBrandIcon();
  const nav=document.getElementById("navbarMount");
  if(nav){
    const loggedIn=typeof isAuthenticated === "function" && isAuthenticated();
    const user=typeof getCurrentUser === "function" ? getCurrentUser() : null;
    const authUi=loggedIn
      ? `<li class="nav-item dropdown auth-nav-item"><a class="nav-link auth-user-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown"><span class="auth-avatar"><i class="bi bi-person-fill"></i></span><span>${user?.name || "Administrator"}</span></a><ul class="dropdown-menu dropdown-menu-end auth-dropdown"><li><div class="auth-dropdown-head"><strong>${user?.name || "Administrator"}</strong><small>${user?.role || "Administrator"}</small></div></li><li><hr class="dropdown-divider"></li><li><a class="dropdown-item" href="dashboard.html"><i class="bi bi-grid-1x2"></i> Dashboard</a></li><li><button class="dropdown-item auth-logout" type="button"><i class="bi bi-box-arrow-right"></i> Keluar</button></li></ul></li>`
